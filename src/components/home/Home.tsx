...
export default function Home() {
  ...
  const [_animated, setAnimated] = useState<boolean>(false);
  const [_animatedUns, setAnimatedUns] = useState<boolean>(false);
  const [_animatedPart, setAnimatedPart] = useState<boolean>(false);
  const [_animatedUnsPart, setAnimatedUnsPart] = useState<boolean>(false);
  ...
  const isMobile = phoneRegEx.test(navigator.userAgent);

  // دالة مساعدة لتنسيق رقم السورة/الجزء في روابط الصوتيات
  const formatAudioNum = (num: number) => num < 10 ? `00${num}` : num < 100 ? `0${num}` : `${num}`;

  // دالة موحدة لتشغيل الصوت
  async function playAudio(type: 'surah' | 'part', qareeName: string, number: number) {
    if (!navigator.onLine) console.log("offline: trying cache");
    if (listening) document.querySelector('audio')?.remove();

    const audioEl = document.createElement('audio');
    let src = '';

    if (type === 'surah') {
      const formattedNum = formatAudioNum(number);
      if (qareeName === 'عبد الباسط عبد الصمد') src = `https://server7.mp3quran.net/basit/Almusshaf-Al-Mojawwad/${formattedNum}.mp3`;
      else if (qareeName === 'محمود خليل الحصري') src = `https://server13.mp3quran.net/husr/${formattedNum}.mp3`;
      else if (qareeName === 'محمد صديق المنشاوي') src = `https://server10.mp3quran.net/minsh/${formattedNum}.mp3`;
      else if (qareeName === 'ياسر الدوسري') src = `https://server11.mp3quran.net/yasser/${formattedNum}.mp3`;
    } else {
      // Parts logic
      const prefix = number < 10 ? '0' : '';
      if (qareeName === 'محمود خليل الحصري') src = `https://ia902906.us.archive.org/20/items/AlHossarry/${prefix}${number}.mp3`;
      else if (qareeName === 'محمد صديق المنشاوي') src = `https://ia801903.us.archive.org/33/items/12_202007xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx31/%D8%A7%D9%84%D9%85%D9%86%D8%B4%D8%A7%D9%88%D9%8A%20%20%D9%85%D8%B1%D8%AA%D9%84%20%D9%85%D8%B5%D8%AD%D9%81%20%20%D9%85%D9%82%D8%B3%D9%85%20%D8%A3%D8%AC%D8%B2%D8%A7%D8%A1%20%D8%AC%D8%B2%D8%A1%20%D8%B1%D9%82%D9%85%20${prefix}${number}.mp3`;
      else if (qareeName === 'عبد الباسط عبد الصمد') src = `https://ia803201.us.archive.org/6/items/aaaaaaaaaaaaaaaaaaaaaa812_20200812_1445/%D9%85%D8%B5%D8%AD%D9%81%20%D8%B9%D8%A8%D8%AF%D8%A7%D9%84%D8%A8%D8%A7%D8%B3%D8%B1%20%D8%A7%D9%84%D9%85%D8%AC%D9%88%D8%AF%20%D9%85%D9%82%D8%B3%D9%85%20%D8%A3%D8%AC%D8%B2%D8%A7%D8%A1%20%D8%AC%D8%B2%D8%A1%20%D8%B1%D9%82%D9%85%20${prefix}${number}.mp3`;
      else if (qareeName === 'ياسر الدوسري') src = `https://ia801909.us.archive.org/19/items/a2aaaaaaaaaaaaaaaaaaaaaaaaaaaaa0200813/%D9%85%D8%B5%D8%AD%D9%81%20%D9%8A%D8%A7%D8%B3%D8%B1%20%D8%A7%D9%84%D8%AF%D9%88%D8%B3%D8%B1%D9%8A%20%D9%85%D9%82%D8%B3%D9%85%20%D8%A3%D8%AC%D8%B2%D8%A7%D8%A1%20%D8%AC%D8%B2%D8%A1%20%D8%B1%D9%82%D9%85%20${prefix}${number}.mp3`;
    }

    if (src) {
      audioEl.src = src;
      audioEl.controls = true;
      (type === 'surah' ? ref : ref2).current?.append(audioEl);
      await audioEl.play();
      setListening(true);
      audioEl.scrollIntoView({ block: "center", behavior: 'smooth' });
    }
  }

  // تهيئة التطبيق (localStorage, Supabase, Werd)
  useEffect(() => {
    handleClick("home", "home"); // Set initial active tab

    const initApp = async () => {
      // 1. Initialize LocalStorage if missing
      if (!localStorage.id) localStorage.setItem("id", crypto.randomUUID());
      if (!localStorage.getItem("current_surah")) localStorage.setItem("current_surah", JSON.stringify([{ surah_num: null, scroll_top: null }]));
      if (!localStorage.getItem("current_part")) localStorage.setItem("current_part", JSON.stringify([{ part_num: null, scroll_top: null }]));
      if (!localStorage.getItem("werd")) localStorage.setItem("werd", JSON.stringify([{ started: false, current_werd: '', last_time: null, index: null }]));

      const userId = localStorage.id;

      // 2. Fetch/Sync Data
      if (isMobile) {
        // Mobile: Prefer LocalStorage
        const lsSurah = JSON.parse(localStorage.current_surah)[0];
        const lsPart = JSON.parse(localStorage.current_part)[0];
        const lsWerd = JSON.parse(localStorage.werd)[0];

        if (lsSurah.surah_num) {
            setCurrentSurahNass(lsSurah.surah_num);
            scrollToElement('.nass', lsSurah.scroll_top);
        }
        if (lsPart.part_num) {
            setCurrentPartNass(lsPart.part_num);
            scrollToElement('.quran-part-nass', lsPart.scroll_top);
        }
        if (lsWerd.started) {
             setStartedWerd(lsWerd.started);
             setDailyWerd(lsWerd.current_werd);
             setCurrentWerd(lsWerd.index);
             checkWerdUpdate(lsWerd, true);
        }
      } else {
        // Desktop: Prefer Supabase
        const { data: surahData } = await supabase.from('current_surah').select().eq('id', userId);
        if (!surahData?.length) {
          await supabase.from('current_surah').insert([{ id: userId, surah_num: null, scroll_top: null }]);
        } else if (surahData[0].surah_num) {
          setCurrentSurahNass(surahData[0].surah_num);
          scrollToElement('.nass', surahData[0].scroll_top);
        }

        const { data: partData } = await supabase.from('current_part').select().eq('id', userId);
        if (!partData?.length) {
           await supabase.from('current_part').insert([{ id: userId, part_num: null, scroll_top: null }]);
        } else if (partData[0].part_num) {
           setCurrentPartNass(partData[0].part_num);
           scrollToElement('.quran-part-nass', partData[0].scroll_top);
        }

        const { data: werdData } = await supabase.from('werd').select().eq('id', userId);
        if (!werdData?.length) {
           await supabase.from('werd').insert([{ id: userId, started: false, current_werd: '', index: null, last_time: null }]);
        } else {
           setStartedWerd(werdData[0].started);
           setDailyWerd(werdData[0].current_werd);
           setCurrentWerd(werdData[0].index);
           checkWerdUpdate(werdData[0], false);
        }
      }
    };
  ...
  // دالة مساعدة للتمرير (Scrolling)
  const scrollToElement = (selector: string, top: number) => {
    let int = setInterval(() => {
        let el = document.querySelector(selector) as HTMLDivElement;
        if (el) {
  ...
  // التحقق من تحديث الورد اليومي
  const checkWerdUpdate = async (data: any, isLocalStorage: boolean) => {
    const currentTime = new Date().getTime();
    if (currentTime - data.last_time >= 24 * 60 * 60 * 1000) {
        let newIndex = data.index + 1;
        const limits: {[key: string]: number} = { 'page': 603, 'hezb': 59, 'part': 29 };
        
        if (data.index === limits[data.current_werd]) newIndex = 0;

        const newData = { last_time: currentTime, index: newIndex };
        
        if (isLocalStorage) {
            const updated = [{ ...data, ...newData }];
            localStorage.setItem("werd", JSON.stringify(updated));
        } else {
            await supabase.from('werd').update([newData]).eq('id', localStorage.id);
        }
  ...
  async function save() {
    ...
    if (isMobile) {
      localStorage.setItem("current_surah", JSON.stringify([{ surah_num: currentSurahNass, scroll_top: scroll }]));
    }
    await supabase.from('current_surah').update({ surah_num: currentSurahNass, scroll_top: scroll }).eq('id', localStorage.id);
    triggerAnimation(setAnimated, savedRef);
  }
  ...
  async function unsave() {
    ...
    triggerAnimation(setAnimatedUns, unsavedRef);
  }
  ...
  async function savePart() {
    ...
    const scroll = el.scrollTop;
    if (isMobile) {
      localStorage.setItem("current_part", JSON.stringify([{ part_num: currentPartNass, scroll_top: scroll }]));
    }
    await supabase.from('current_part').update({ part_num: currentPartNass, scroll_top: scroll }).eq('id', localStorage.id);
    triggerAnimation(setAnimatedPart, savedRefPart);
  }
  ...
  async function unsavePart() {
    ...
    triggerAnimation(setAnimatedUnsPart, unsavedRefPart);
  }

  const triggerAnimation = (setAnim: Function, ref: any) => {
    setAnim(true);
    setTimeout(() => {
  ...
  async function startWerd(werd: string) {
    ...
    const payload = { started: true, current_werd: werd, last_time: date, index: 0 };
    
    await supabase.from('werd').update([payload]).eq('id', localStorage.id);
    if (isMobile) {
      localStorage.setItem("werd", JSON.stringify([payload]));
    }
    setWerdModal(false);
    setStartedWerd(true);
    setCurrentWerd(0);
  ...
  async function cancelWerd() {
    const payload = { started: false, current_werd: '', last_time: 0, index: 0 };
    await supabase.from("werd").update(payload).eq("id", localStorage.id);
    if (isMobile) {
      localStorage.setItem("werd", JSON.stringify([payload]));
    }
  ...
  function handleClick(className: string, newPage: string) {
    const allElements = [
      document.getElementsByClassName('home')[0] as SVGElement, document.getElementsByClassName('home')[1] as HTMLDivElement,
      document.getElementsByClassName('search')[0] as SVGElement, document.getElementsByClassName('search')[1] as HTMLDivElement,
      document.getElementsByClassName('azkar')[0] as SVGElement, document.getElementsByClassName('azkar')[1] as HTMLDivElement,
      document.getElementsByClassName('counter')[0] as SVGElement, document.getElementsByClassName('counter')[1] as HTMLDivElement,
    ];
    ...
    if(el1) el1.setAttribute("fill", "#b8860b");
    if(el2) el2.style.color = '#a89460ff';
  ...
  function getPageImage(index: number) {
    ...
    return new URL(`../../data/werd/pages/page${padded}.png`, import.meta.url).href;
  }
  ...
  return (
    <>
      ...
      <div id="app">
          ...
          <div className="quran">
            <div className="fehres">
              ...
              {allSurah_s.filter(surah => surah.replace("أ", "ا").replace("آ", "ا").includes(surahSearchValue.replace("أ", "ا").replace("آ", "ا"))).map((surah, i) => (
                <div className="surah" key={i} onClick={() => { setCurrentSurahNass(i + 1); scrollToElement('.nass', 0); }}>
                  <div className="surah-number">{i + 1}</div>
                  <div className="surah-name">سورة {surah}</div>
                </div>
              ))}
            </div>
            {currentSurahNass > 0 &&
              <div className="nass">
                <div className="surah-nass-name">سورة {allSurah_s[currentSurahNass - 1]}</div>
            ...
            <>
              <div className="listen-surah">
                ...
                <div className="buttons">
                  <div className="next">
                    <button disabled={currentSurahNass === 114} onClick={() => { setCurrentSurahNass(c => c + 1); scrollToElement('.nass', 0); }}>السورة التالية</button>
                  </div>
                  <div className="prev">
                    <button disabled={currentSurahNass === 1} onClick={() => { setCurrentSurahNass(c => c - 1); scrollToElement('.nass', 0); }}>السورة السابقة</button>
                  </div>
                </div>
                <div className="open-reading-mode" onClick={() => openReadingMode("surah")}>وضع القراءة</div>
                <div className="listen" onClick={() => setListen(true)}>الاستماع إلى سورة {allSurah_s[currentSurahNass - 1]}</div>
              </div>
            </>
          }

          <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
            <div className="saved" ref={savedRef}>!تم حفظ السورة بنجاح</div>
          </div>
          <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
            <div className="unsaved" ref={unsavedRef}>!تم إلغاء حفظ السورة بنجاح</div>
          </div>

          {listen && 
            <div className="choose-qaree-modal" onClick={() => setListen(false)}>
              <div className="choose-qaree">
                <div className="choose-qaree-title">اختر أحد القراء</div>
                <div className="quraa">
                  {quraa.map((qaree, i) => (
                    <div className="qaree" key={i} onClick={() => playAudio('surah', qaree.name, currentSurahNass)}>
                      <div className="qaree-image"><img src={qaree.image} alt={qaree.name} /></div>
                      <div className="qaree-name">{qaree.name}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="audio" ref={ref}></div>
        <div className="audio" ref={ref2}></div>
        ...
        {currentPage === 'home' && <>
          <div className="parts">
            <div className="quran-parts">
              ...
              {Array.from({ length: 30 }).map((_x, i) => (
                <div className="quran-part" key={i} onClick={() => { setCurrentPartNass(i + 1); scrollToElement('.quran-part-nass', 0); }}>
                  الجزء {i + 1}
                </div>
              ))}
            </div>
            {currentPartNass > 0 &&
              <div className="quran-part-nass">
                <div className="quran-part-nass-name">الجزء {currentPartNass}</div>
                <div className="quran-part-nass-text">{allParts[currentPartNass - 1]}</div>
              </div>
            }
          </div>
          ...
          {currentPartNass > 0 &&
            <div className="listen-surah">
              ...
              <div className="buttons">
                <div className="next">
                  <button disabled={currentPartNass === 30} onClick={() => { setCurrentPartNass(c => c + 1); scrollToElement('.quran-part-nass', 0); }}>الجزء التالي</button>
                </div>
                <div className="prev">
                  <button disabled={currentPartNass === 1} onClick={() => { setCurrentPartNass(c => c - 1); scrollToElement('.quran-part-nass', 0); }}>الجزء السابق</button>
                </div>
              </div>
              <div className="open-reading-mode" onClick={() => openReadingMode("part")}>وضع القراءة</div>
              <div className="listen" onClick={() => setListenPart(true)}>الاستماع إلى الجزء {currentPartNass}</div>
            </div>
          }
          ...
          {listenPart &&
            <div className="choose-qaree-modal" onClick={() => setListenPart(false)}>
              <div className="choose-qaree">
                ...
                <div className="quraa">
                  {quraa.map((qaree, i) => (
                    <div className="qaree" key={i} onClick={() => playAudio('part', qaree.name, currentPartNass)}>
                      <div className="qaree-image"><img src={qaree.image} alt={qaree.name} /></div>
                      <div className="qaree-name">{qaree.name}</div>
                    </div>
                  ))}
          ...
          <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
            <div className="saved saved_part" ref={savedRefPart}>!تم حفظ الجزء بنجاح</div>
          </div>
          <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
            <div className="unsaved saved_part" ref={unsavedRefPart}>!تم إلغاء حفظ الجزء بنجاح</div>
          </div>
          ...
          {werdModal &&
            <div className="werd-modal-container">
              <div className="werd-modal">
                <div className="werd-modal-title">اختر وردك اليومي</div>
                <div className="werd-options">
                  <div className="werd-option" onClick={() => startWerd('page')}>صفحة</div>
                  <div className="werd-option" onClick={() => startWerd('hezb')}>حزب</div>
                  <div className="werd-option" onClick={() => startWerd('part')}>جزء</div>
                </div>
              </div>
            </div>
          }
          ...
          {startedWerd && <>
            <div className="daily-werd">
              <div className="werd-container">
                <div className="werd-title">ورد اليوم</div>
                <div className="werd-text">
                  {dailyWerd === 'page' && <img src={getPageImage(currentWerd)} alt="werd" className="werd-img" />}
        ...
        <div className="tools">
          <div className="tool" onClick={() => { handleClick('ramadan', 'ramadan');navigate("/ramadan") }}>
            <div className="tool-img">
            <svg className="ramadan" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="120" height="50" fill="#fff">
            ...
            <div className="tool-name ramadan">
              رمضان
            </div>
          </div>
          <div className="tool" onClick={() => { handleClick('counter', 'counter') }}>
            ...
            <div className="tool-name counter">المسبحة الإلكترونية</div>
          </div>
          <div className="tool" onClick={() => { handleClick('azkar', 'azkar') }}>
            <div className="tool-img">
              <svg xmlns="http://www.w3.org/2000/svg" className="azkar" version="1.0" width="120" height="50" viewBox="0 0 1280.000000 1118.000000" preserveAspectRatio="xMidYMid meet" fill="#fff">
            ...
            <div className="tool-name azkar">الأذكار</div>
          </div>
          <div className="tool" onClick={() => { handleClick('search', 'search') }}>
            <div className="tool-img">
              <svg xmlns="http://www.w3.org/2000/svg" className="search" width="120" height="50" viewBox="0 0 24 24" fill="#fff">
                <path d="M 9 2 C 5.1458514 2 2 5.1458514 2 9 C 2 12.854149 5.1458514 16 9 16 C 10.747998 16 12.345009 15.348024 13.574219 14.28125 L 14 14.707031 L 14 16 L 20 22 L 22 20 L 16 14 L 14.707031 14 L 14.28125 13.574219 C 15.348024 12.345009 16 10.747998 16 9 C 16 5.1458514 12.854149 2 9 2 z M 9 4 C 11.773268 4 14 6.2267316 14 9 C 14 11.773268 11.773268 14 9 14 C 6.2267316 14 4 11.773268 4 9 C 4 6.2267316 6.2267316 4 9 4 z"></path>
              </svg>
            </div>
            <div className="tool-name search">الباحث القرآني</div>
          </div>
          <div className="tool first" onClick={() => { handleClick('home', 'home') }}>
            <div className="tool-img">
              <svg xmlns="http://www.w3.org/2000/svg" className="home" width="120" height="50" viewBox="0 0 24 24" fill="#fff">
                <path d="M 12 2 A 1 1 0 0 0 11.289062 2.296875 L 1.203125 11.097656 A 0.5 0.5 0 0 0 1 11.5 A 0.5 0.5 0 0 0 1.5 12 L 4 12 L 4 20 C 4 20.552 4.448 21 5 21 L 9 21 C 9.552 21 10 20.552 10 20 L 10 14 L 14 14 L 14 20 C 14 20.552 14.448 21 15 21 L 19 21 C 19.552 21 20 20.552 20 20 L 20 12 L 22.5 12 A 0.5 0.5 0 0 0 23 11.5 A 0.5 0.5 0 0 0 22.796875 11.097656 L 12.716797 2.3027344 A 1 1 0 0 0 12.710938 2.296875 A 1 1 0 0 0 12 2 z"></path>
              </svg>
            </div>
            <div className="tool-name home">الرئيسية</div>
          </div>
        </div>
      </div>
    </>
